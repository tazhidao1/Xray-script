...
function build_warp() {
  if ! docker images --format "{{.Repository}}" | grep -q xray-script-warp; then
    ...
    mkdir -p ${HOME}/.warp
    wget -O /usr/local/xray-script/warp/Dockerfile https://raw.githubusercontent.com/tazhidao1/Xray-script/main/cloudflare-warp/Dockerfile || print_error "WARP Dockerfile 下载失败"
    wget -O /usr/local/xray-script/warp/startup.sh https://raw.githubusercontent.com/tazhidao1/Xray-script/main/cloudflare-warp/startup.sh || print_error "WARP startup.sh 下载失败"
...
function install_cloudreve() {
  if ! docker ps --format "{{.Names}}" | grep -q "^cloudreve\$"; then
    ...
    wget -O /usr/local/cloudreve/docker-compose.yaml https://raw.githubusercontent.com/tazhidao1/Xray-script/main/cloudreve/docker-compose.yaml
